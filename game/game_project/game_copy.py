...
# Two players simultaneously:
    We will have two players play at the time.
...
# Timer:
    ...
    When one player finishes tapping all of the tiles, the game will end.
    The time taken by that player will be displayed on the screen

# Music/Sound effects:
    when the player tap the black keys, we will try to add some piano sound.
    If it gets dissonant when two players tap, we will just add some piano music as the background music when the game starts.
...
def splash(keys):
    ...
    camera.draw(gamebox.from_text(camera.x, camera.y - 130,
                                  ...
                                  "white"))
    keyboard = gamebox.from_image(camera.x, camera.y - 40, 'https://www3.northern.edu/wieland/piano/1st/keyboard.gif')
    keyboard.width = 150
    camera.draw(keyboard)
    camera.draw(gamebox.from_text(camera.x, camera.y + 50,
                                  "PLAYER 1: Press 'A','S','D','F'", "Arial", 30,
                                  "white"))
    camera.draw(gamebox.from_text(camera.x, camera.y + 100,
                                  "PLAYER 2: Press 'H','J','K','L'", "Arial", 30,
                                  "white"))
...
def tick(keys):
    ...
    for tile in left_tiles:
        ...
        if start:
            if tile.y == 550:
                if tile.x == 50 and pygame.K_a in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in left_tiles:
                        tile.y += 100
                    left_number += 1
                if tile.x == 150 and pygame.K_s in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in left_tiles:
                        tile.y += 100
                    left_number += 1
                if tile.x == 250 and pygame.K_d in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in left_tiles:
                        tile.y += 100
                    left_number += 1
                if tile.x == 350 and pygame.K_f in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in left_tiles:
                        tile.y += 100
                    left_number += 1
    ...
    for tile in right_tiles:
        ...
        if start:
            if tile.y == 550:
                if tile.x == 450 and pygame.K_h in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in right_tiles:
                        tile.y += 100
                    right_number += 1
                if tile.x == 550 and pygame.K_j in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in right_tiles:
                        tile.y += 100
                    right_number += 1
                if tile.x == 650 and pygame.K_k in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in right_tiles:
                        tile.y += 100
                    right_number += 1
                if tile.x == 750 and pygame.K_l in keys and last_press < ticks - 2:
                    last_press = ticks
                    for tile in right_tiles:
                        tile.y += 100
                    right_number += 1
